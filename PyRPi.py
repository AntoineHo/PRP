...
class FileHandler:
    ...
    def __init__(self, dir):
        ...
        # Set the dir path to dir/working_directory
    ...
    def move_to_input(self):
        ...
        # Empties infiles
        self._infiles = []
        # Fills _infiles with moved files
        for file in listdir:
    ...
    def run_fastqc(self, files=_infiles, zip_extract=False):
        # First checks output directory
        self.check_output_dir()
        # Then creates a subdirectory for fastqc in output and input
        fastqc_outdir = self.dir + "/output/fastqc/"
        os.makedirs(fastqc_outdir, exist_ok = True)
        
        # Checks files in infiles
        self.check_files()
        
        # Returns to working directory if it was not in
        os.chdir(self.dir)
        ...
        # Adds outdir location to the command
        fa_cmd += "--outdir=" + fastqc_outdir
        
        # Runs the fa_cmd generated
        print("--Running fastQC...")
        subprocess.run(fa_cmd, shell=True, check=True)
        print("fastQC done--\n")
